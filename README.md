...
The top bar switches between two modes: **AI coding** (automatic analysis) and
**Human coding** (manual coding for reliability checks).
<br>상단 바에서 **AI 코딩**(자동 분석)과 **인간 코딩**(신뢰도 검증용 수기 코딩) 두
모드를 전환합니다.

### AI coding mode / AI 코딩 모드
...
### Human coding mode / 인간 코딩 모드

Switch to **Human coding** in the top bar to code the same windows by hand — no
AI run required (just load a video). It is meant for measuring inter-coder
reliability against the AI output.
<br>상단 바에서 **인간 코딩**으로 전환하면 같은 윈도우를 직접 코딩할 수 있습니다.
**분석 시작을 누를 필요 없이** 영상만 불러오면 되며, AI 결과와의 코더 간 신뢰도 측정을
위한 모드입니다.

1. Enter the **coder name**. / **코더 이름**을 입력한다.
2. Step through windows with **◀ / ▶**; **Play this window** plays only that segment and pauses at its end. / **◀ / ▶**로 윈도우를 이동하고, **이 구간 재생**으로 해당 구간만 재생(끝에서 자동 정지).
3. Set the **playback speed** (1.0 / **0.8** / 0.5×). / **배속**(1.0 / **0.8** / 0.5배) 선택.
4. Click the **code buttons** (each shows a short gloss); leave empty to record GT-N. / **코드 버튼**(짧은 해설 표시) 클릭, 비우면 GT-N으로 기록.
5. **Save & next** confirms the window and advances; **Human coding CSV** saves `results/{video}_human_{coder}_{datetime}.csv`. / **저장 & 다음**으로 확정·이동, **인간 코딩 CSV**로 `results/{영상}_human_{코더}_{날짜시간}.csv` 저장.

> Windows match the AI windows (same interval/size/start), so the two CSVs align
> for reliability analysis. / 윈도우는 AI와 동일(간격·크기·시작점)하게 구성되어 두
> CSV가 신뢰도 분석 시 정렬됩니다.
...
Human coding exports `results/{video}_human_{coder}_{datetime}.csv` with columns
`no, timestamp, coder, gesture`, aligned to the AI windows for reliability
analysis. / 인간 코딩은 `no, timestamp, coder, gesture` 열의
`results/{영상}_human_{코더}_{날짜시간}.csv`로 내보내지며, AI 윈도우와 정렬됩니다.
